        ...
        public void updateRoomTAB() {
            if (!this.player.isBot()) {
                ...
                if (this.player.isInMatch()) {
                    header = String.join("\n", RealSkywarsAPI.getInstance().getLanguageManagerAPI().getList(this.player, LanguageManagerAPI.TL.TAB_HEADER_MATCH)).replace("%map%", this.player.getMatch().getName()).replace("%players%", this.player.getMatch().getPlayers().size() + "").replace("%space%", Text.makeSpace());
                    footer = String.join("\n", RealSkywarsAPI.getInstance().getLanguageManagerAPI().getList(this.player, LanguageManagerAPI.TL.TAB_FOOTER_MATCH)).replace("%map%", this.player.getMatch().getName()).replace("%players%", this.player.getMatch().getPlayers().size() + "").replace("%space%", Text.makeSpace());
                } else {
                    ...
                    if (p.getState() != null) {
                        switch (p.getState()) {
                            ...
                            case CAGE:
                                ...
                                tit = RealSkywarsAPI.getInstance().getLanguageManagerAPI().getString(p, LanguageManagerAPI.TS.SCOREBOARD_CAGE_TITLE, false).replace("%map%", p.getMatch().getName()).replace("%mode%", p.getMatch().getGameMode().name());
                                break;
                            case SPECTATOR:
                            case EXTERNAL_SPECTATOR:
                                ...
                                tit = RealSkywarsAPI.getInstance().getLanguageManagerAPI().getString(p, LanguageManagerAPI.TS.SCOREBOARD_SPECTATOR_TITLE, false).replace("%map%", p.getMatch().getName()).replace("%mode%", p.getMatch().getGameMode().name());
                                break;
                            case PLAYING:
                                lista = RealSkywarsAPI.getInstance().getLanguageManagerAPI().getList(p, LanguageManagerAPI.TL.SCOREBOARD_PLAYING_LINES);
                                tit = RealSkywarsAPI.getInstance().getLanguageManagerAPI().getString(p, LanguageManagerAPI.TS.SCOREBOARD_PLAYING_TITLE, false).replace("%map%", p.getMatch().getName()).replace("%mode%", p.getMatch().getGameMode().name());
                                break;
                            default:
