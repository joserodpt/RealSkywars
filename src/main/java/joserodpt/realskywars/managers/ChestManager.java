...
import joserodpt.realskywars.chests.SWChestItem;
...
public class ChestManager {
    ...
    public void set2ChestRaw(SWChest.Tier t, SWChest.Type type, List<SWChestItem> itens) {
        ...
        switch (t) {
            case BASIC:
                BasicChestConfig.file().set((type == SWChest.Type.MID) ? "Mid-Items" : "Items", map);
                BasicChestConfig.save();
                break;
            case NORMAL:
                NormalChestConfig.file().set((type == SWChest.Type.MID) ? "Mid-Items" : "Items", map);
                NormalChestConfig.save();
                break;
            case EPIC:
                EPICChestConfig.file().set((type == SWChest.Type.MID) ? "Mid-Items" : "Items", map);
